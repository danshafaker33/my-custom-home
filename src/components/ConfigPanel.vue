          ...
          <div class="music-player-container">
            <div v-if="!musicStore.isAdmin" class="mb-6 pa-4 rounded-lg bg-grey-darken-3 border-thin">
              <div class="text-caption text-grey mb-2">
                {{ musicStore.adminPassword ? '管理员模式已锁定' : '首次使用请先设置管理员密码' }}
              </div>
              <div class="d-flex gap-2">
                <v-text-field
                  v-model="passwordInput"
                  :type="showPassword ? 'text' : 'password'"
                  :placeholder="musicStore.adminPassword ? '输入管理员密码解锁' : '设置管理员密码'"
                  density="compact"
                  variant="outlined"
                  hide-details
                  class="flex-grow-1"
            ...
            <div v-else class="mb-6 pa-3 rounded-lg bg-blue-grey-darken-4 d-flex align-center justify-space-between border-thin">
              <div class="d-flex align-center">
                ...
                <span class="text-caption text-primary font-weight-bold">管理员模式已启用</span>
              </div>
              <div class="d-flex gap-2">
            ...
            <div class="music-player-preview mb-6 pa-4 rounded-lg bg-grey-darken-3 border-thin">
              <div class="d-flex align-center mb-2">
                ...
                <div class="d-flex align-center">
                  <v-btn icon="mdi-skip-previous" variant="text" color="white" class="mr-1" @click="musicStore.prev()"></v-btn>
                  <v-btn
                  ...
                  ></v-btn>
                  <v-btn icon="mdi-skip-next" variant="text" color="white" class="ml-1" @click="musicStore.next()"></v-btn>
                </div>
              </div>
            ...
            <v-expand-transition>
              <div v-if="showAddForm" class="mb-6 pa-4 rounded-lg border-thin">
                ...
                <v-text-field
                  ...
                  hide-details
                ></v-text-field>
                <div class="text-tiny text-grey mb-4 px-1">建议使用网络图片链接，或放在 `public/covers` 目录下。</div>
...
const copyPlaylistCode = () => {
  ...
  navigator.clipboard.writeText(code).then(() => {
    snackbarText.value = '代码已复制到剪贴板，请同步更新 data.js 以永久保存'
    snackbar.value = true
  })
}
...
const handleAddSong = () => {
  ...
  if (newSong.value.url && normalizedUrl === null) {
    musicStore.error = '不支持本地磁盘路径。请将音频复制到项目 public/music 目录下，并使用 /music/文件名.mp3 访问。'
    return
  }

  if (newSong.value.cover && normalizedCover === null) {
    musicStore.error = '不支持本地磁盘封面。请将封面复制到项目 public/covers 目录下，并使用 /covers/文件名.jpg 访问。'
    return
  }
...
.music-slider:hover :deep(.v-slider-thumb__surface) {
  transform: scale(1.2);
}
